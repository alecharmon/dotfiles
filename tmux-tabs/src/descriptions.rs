...
fn capture_window_text<T: Tmux>(t: &T, win_index: &str, home: &str) -> String {
    ...
    t.run(&["capture-pane", "-p", "-t", &pane, "-S", "-200"]).unwrap_or_default()
}

fn describe_with_pi(captured: &str) -> String {
...
fn cache_path(ctx: &Ctx, session_id: &str, win_index: &str) -> std::path::PathBuf {
    ctx.descriptions_dir(session_id)
...
fn cached_record(ctx: &Ctx, session_id: &str, win_index: &str) -> (String, String) {
    ...
    (
        json_field(&text, "signature"),
        json_field(&text, "sample"),
    )
}
...
fn write_description(ctx: &Ctx, session_id: &str, win_index: &str, captured: &str, desc: &str) {
    ...
    let one_line: String = desc.lines().next().unwrap_or("").chars().take(200).collect();
    let body = format!(
        "{{\n  \"description\": \"{}\",\n  \"sample\": \"{}\",\n  \"signature\": \"{}\",\n  \"window\": \"{}\"\n}}\n",
        json_escape(one_line.trim()),
        json_escape(&sample),
        signature(captured),
        win_index
    );
...
pub fn refresh_once(ctx: &Ctx) -> bool {
    ...
    for w in tmux_windows(&t, ctx) {
        ...
        let desc = describe_with_pi(&captured);
        if !desc.is_empty() {
            write_description(ctx, &session_id, &w.index, &captured, &desc);
            changed = true;
        }
    }
    changed
