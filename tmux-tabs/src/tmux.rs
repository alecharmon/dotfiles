...
use crate::model::{project_group, window_display_name, Window};
...
pub fn pane_var<T: Tmux>(t: &T, pane: &str, fmt: &str) -> String {
    t.run(&["display-message", "-p", "-t", pane, fmt]).unwrap_or_default()
}
...
pub fn pane_records<T: Tmux>(t: &T, win_index: &str) -> Vec<PaneRecord> {
    ...
    let out = t.run(&["list-panes", "-t", win_index, "-F", fmt]).unwrap_or_default();
...
pub fn active_pane<T: Tmux>(t: &T, win_index: &str, home: &str) -> String {
    let panes = pane_records(t, win_index);
    representative_pane(&panes, home).map(|p| p.id.clone()).unwrap_or_default()
}
...
pub fn pane_ids_by_window<T: Tmux>(t: &T, home: &str) -> HashMap<String, Vec<String>> {
    let fmt = "#{window_index}|#{pane_id}|#{pane_current_command}|#{pane_title}|#{pane_current_path}";
...
pub fn tmux_windows<T: Tmux>(t: &T, ctx: &Ctx) -> Vec<Window> {
    ...
    for line in out.lines() {
        ...
        let display_path = rep.map(|p| p.path.clone()).filter(|s| !s.is_empty()).unwrap_or_else(|| path.to_string());
        let display_title = rep.map(|p| p.title.clone()).filter(|s| !s.is_empty()).unwrap_or_else(|| title.to_string());
...
fn description_for_window(ctx: &Ctx, session_id: &str, win_index: &str) -> String {
    let path = ctx
        .descriptions_dir(session_id)
        .join(format!("{}-{}.json", session_id.trim_start_matches('$'), win_index));
...
pub fn clear_ready_panes(ctx: &Ctx, panes: &[String]) {
    for pane in panes {
        let file = ctx.agent_ready_dir().join(format!("{}.json", pane.trim_start_matches('%')));
